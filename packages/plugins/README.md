...
The canonical order is in [`createCorePlugins()`](src/index.ts). Hooks run in the order in `@svartz/core`'s `STAGE_NAMES`; `enforce: 'pre' | 'post'` changes order within one hook. Mutating transforms run serially. The emitter compiles `.svx` as authored Svelte. `.md`, `.markdown`, and `.mdx` remain inert Markdown.
