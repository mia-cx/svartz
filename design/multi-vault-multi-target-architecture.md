...
### Option A: Multi-build orchestrator (recommended for now)

1. **Config parser** reads `targets` array.
2. **Build orchestrator** script (in `packages/build-orchestrator/` or as a CLI):
   ```bash
   svartz build --config svartz.config.ts
   ```
3. For each target:
   - Set env vars: `VAULT_DIR=docs`, `ROOT_PATH=/docs`, `OUTPUT_DIR=build/docs`
   - Invoke SvelteKit build: `npm run build --output=$OUTPUT_DIR`
   - SvelteKit produces `build/docs/` (static or Worker bundle)
4. All outputs in `build/`.

### Option B: Vite plugin orchestration

- Vite plugin discovers targets from config at startup.
- Runs multiple build passes (or uses Vite's multi-config).
- Outputs separate bundles per vault.
...
- **Size limits:** If a vault + SvelteKit bundle exceeds Worker limits (10 MB uncompressed), split to sub-workers or use streaming?
- **Cross-vault links:** Can notes in one vault link to docs in another? (Complicates graph; may need to keep vaults isolated for now.)
- **Shared dependencies:** If both vaults use the same plugins/transforms, can we share the compiled pipeline or do we rebuild for each?
- **Single app vs. multiple apps:** One SvelteKit app with env-driven routing vs. separate apps per target? (One app simpler; multiple apps cleaner separation but more config.)
