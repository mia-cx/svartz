...
- `src/lib/svartz/testing/fixtures/runtime-host.ts`
- `src/lib/svartz/testing/fixtures/shared.ts`

`vite.config.ts` aliases the theme, artifacts, and host virtual modules to those files only when `VITEST` is set. Real runtime builds point them at generated files under `.svartz/vaults/<vault-id>/artifacts/` and `.svartz/host/`.
...
- The catchall route awaits its selected theme in universal `load` before `SvartzRuntimePage` renders. This also prepares lazy theme pages for hydration.
