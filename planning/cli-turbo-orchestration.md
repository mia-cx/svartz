...
The copyable workflows live in [docs/ci.md](../docs/ci.md). Run one `pnpm i --frozen-lockfile` at the repository root. `actions/setup-node` caches the pnpm store; `actions/cache` saves `.turbo` under a commit-specific key with a restore prefix. GitHub caches are immutable, so a constant key cannot receive later build results. Fresh installs recreate `node_modules` from the lockfile and cached store. A scaffolded project uses npm and has no Turbo layer.
