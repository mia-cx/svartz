...
- [x] Review & Close: inspect the full diff, confirm checks, and file a stacked PR for #20.
...
- Final check: `pnpm release:check` passed, including packed fresh and Vite 8 host build/dev. `pnpm exec turbo test --concurrency=2` passed all 16 tasks. The unrestricted `pnpm test` run filled swap and caused timeouts; isolated tests passed before the limited-concurrency run.
