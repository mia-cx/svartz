...
Inline `#topic` and nested `#topic/subtopic` tags become links to the vault's tag routes. They join frontmatter tags in listings and search. Numeric-only tags and tags inside code are left alone. Publication filtering runs before tag extraction, so drafts and private notes do not add public tags.
