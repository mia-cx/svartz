...
Fenced code-block slots receive a `language` prop, such as `typescript`, with or without syntax highlighting. A theme that replaces the code-block slot should preserve the rendered children and HTML attributes to keep highlighting and Mermaid diagrams functional.
